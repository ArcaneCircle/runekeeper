  ...
  function render(ctx) {
    retainTransform(() => {
      ...
      retainTransform(() => {
        ...
        if (state == ATTACK) {
          ctx.translate(
            -Math.exp(-Math.pow((timeInState - 0.25) * 1.5, 2) * 25) * 20,
            0,
          );
          ctx.rotate(
            -Math.exp(-Math.pow((timeInState - 0.3) * 1.5, 2) * 30) * 1.2,
          );
        }
        if (enemyType == 0) {
          ctx.rotate(-0.2);
      ...
      if (motion == -1) {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          const w = 1 - p * p * p * p * p;
          ctx.fillStyle = `rgba(${250 - 150 * p}, ${250 - p * 150}, ${255}, ${
            (1 - p * p) * p * 4
          })`;
          retainTransform(() => {
            ctx.beginPath();
      ...
      if (onFire > 0) {
        for (let i = 0; i < 9; i++) {
          ...
          const q = 0.15 + p * 0.85;
          const w = 1 - p * p * p * p * p;
          ctx.fillStyle = `rgba(${255 * w}, ${(50 + p * 200) * w}, ${30 * w}, ${
            1 - p
          })`;
          ctx.beginPath();
          ctx.ellipse(
