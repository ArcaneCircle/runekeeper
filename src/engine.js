...
function tick(currentFrameMs) {
  ...
  const zoom = Math.min(canvas.width / 800, canvas.height / 1100);
  ctx.setTransform(
    zoom,
    0,
    0,
    zoom,
    canvas.width / 2 - 190 * zoom,
    canvas.height * 0.3 - 80 * 6 * 0.5 * zoom,
  );
...
export { add, remove, clear, resort, getObjectsByTag };
