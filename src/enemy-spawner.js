...
function EnemySpawner() {
  ...
  function spawnOne() {
    ...
    if (enemies.length < 3) {
      const MAX_ALLOWED = parseInt(
        Math.min(getObjectsByTag("game")[0].getScore() / 4, 2),
      );
      const enemyType = parseInt(
        Math.min(Math.floor(Math.random() * (MAX_ALLOWED + 1)), MAX_ALLOWED),
      );
      for (let i = 0; i < 30; i++) {
