...
function Audio() {
  ...
  async function init() {
    ...
    runestoneLandedSound = await generate(0.25, (i) => {
      return (
        0.1 *
        (sqr(i / (120 + i / 250)) * 0.3 + Math.random()) *
        (sqr(i / 600) * 0.5 + 0.5)
      );
    });
    ...
    iceSound = await generate(0.4, (i) => {
      return (
        0.11 *
        (sqr(i / (30 + i / 500)) *
          (sqr(i / (1500 + i / 60)) * 0.5 + 0.5) *
          (sqr(i / (600 + i / 40)) * 0.5 + 0.5))
      );
    });
    ...
    function sigilNote(i, pitch, time) {
      ...
      return (
        0.1 *
        (saw(i / q) * sin(i / (q * 2 + 3 * sin(i / 200000)))) *
        (i / 44100 - time > 0 ? 1 : 0) *
        Math.exp(-(i / 44100 - time) * 7)
      );
    }
    ...
    sigilTriangle = await generate(2.0, (i) => {
      return (
        sigilNote(i, 10, 0) + sigilNote(i, 10, 0.15) + sigilNote(i, 3, 0.3)
      );
    });
    ...
    sigilGarbage = await generate(2.0, (i) => {
      return (
        sigilNote(i, -4, 0) + sigilNote(i, -10, 0.15) + sigilNote(i, -4, 0.3)
      );
    });
    ...
    for (let j = 0; j < W; j++) {
      drumBuffer[j] +=
        0.06 * (sin(j / (70 + j / 300)) + Math.random() / 3) * (1 - j / W);
      drumBuffer[parseInt(0.25 * pace) + j] +=
        0.02 * (saw(j / (80 - j / 2000)) + Math.random() / 3) * (1 - j / W);
      drumBuffer[parseInt(0.5 * pace) + j] +=
        0.06 * Math.random() * (1 - j / W) +
        0.06 * (sin(j / (70 + j / 300)) + Math.random() / 3) * (1 - j / W);
      drumBuffer[parseInt(0.75 * pace) + j] +=
        0.02 * (saw(j / (80 - j / 2000)) + Math.random() / 3) * (1 - j / W);
    }
    await _yield();
    ...
    function chordNote(j, pitch) {
      ...
      return (
        0.04 *
        sin(j / (p + Math.exp((-j / U) * 4) * 5)) *
        clamp(1 - j / (U * 12), 0, 1) *
        Math.pow(sin(j / 1550) * 0.5 + 0.5, 3)
      );
    }
  ...
  async function genericSongBuilder(
    [melodySignature, beat],
    seed,
    prog1,
    prog2,
  ) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        song.push([
          bassNotes[(seed * 7 + i * 2 + (j >> 1) + j * j * 3) % 4] + q,
          j + o,
          6,
          1,
        ]);
        ...
        if ((j + j * j + i + seed * 3) % 7 < 4) {
          song.push([
            -3 +
              q +
              melodySignature[
                (j + j * j * 2 + i * i * 2 + seed) % melodySignature.length
              ],
            j * noteSpace + o,
            noteLength,
            2,
    ...
    for (let i = 0; i < song.length; i++) {
      ...
      for (let i = 0; i < dur; i++) {
        ...
        v +=
          amp == 1
            ? clamp(
                sin(i / (6 * 2 ** (-note / 12) * 2 * 2) + sin(i / 8000)) *
                  (Math.exp(-envelope * 23) * 44 + 1),
                -1,
                1,
              ) * 2
        ...
        buffer[baseIdx + i] +=
          (v *
            Math.min(
              envelope * Math.exp(-envelope * (10 + amp * 7)) * 100,
              1,
            )) /
          500;
      }
